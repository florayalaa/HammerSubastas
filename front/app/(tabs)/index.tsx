...
import { Gavel, DollarSign, ChevronRight } from 'lucide-react-native';
    ...
    const fetchAuctions = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // Mapeamos datos del backend al formato del frontend
          const mapped = data.map((a: any) => ({
            id: a.id,
            title: a.title,
            date: new Date(a.startDate).toLocaleDateString() + ' - ' + new Date(a.startDate).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}),
            category: 'Oro', // Fallback: la categoría puede no estar en la subasta directamente
            items: 1, // Fallback
  ...
  return (
    <ScrollView className="flex-1 bg-gray-50 px-4 py-4" showsVerticalScrollIndicator={false}>
      ...
      <View className="mb-6 flex-row items-center justify-between">
        <View>
          <Text className="text-2xl font-bold text-[#333F48]">Inicio</Text>
        </View>
